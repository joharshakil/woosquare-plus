...
class WooSquare_Gateway extends WC_Payment_Gateway { 
	...
	public function admin_notices() {
		...
		if ( ! WOOSQU_ENABLE_STAGING && get_option( 'woocommerce_force_ssl_checkout' ) === 'no' && ! class_exists( 'WordPressHTTPS' ) ) {
	...
	public function payment_scripts() {
		...
		$location = get_option('woo_square_location_id_free');
		if($woocommerce_square_settings['enable_sandbox'] == 'yes'){
	...
	public function process_payment( $order_id, $retry = true ) {
		...
		if(
		!empty($_POST['_wcf_checkout_id']) 
			and 
		!empty($_POST['_wcf_flow_id'])
			OR 
		$this->maybe_process_pre_orders( $order_id )
			...
			$_square_customer_id = get_user_meta( $customer_id, '_square_customer_id', true );
			...
			$data = array(
			'customer_id' => $_square_customer_id,
			'card_nonce' => $nonce,
		...
		$result = $this->connect->create_customerCard( $data );
		if(!empty($result->card->id)){
			$_woos_plus_customer_card_id = $result->card->id;
			update_post_meta($order_id, '_woos_plus_customer_card_id',$_woos_plus_customer_card_id);			
		}	
		...
		try {
			...
			if(
				...
				$fields['source_id'] = $_woos_plus_customer_card_id;
			...
			if ( ! empty( $result->errors ) ) {
				...
				if ( 'PAYMENT_METHOD_ERROR' === $result->errors[0]->category || 'VALIDATION_ERROR' === $result->errors[0]->category ) {
					...
					foreach( $result->errors as $error ) {
					    if (strpos($error->detail, 'ADDRESS_VERIFICATION_FAILURE') !== false){
							$error_html .= '<li>Invalid zip code!</li>';
				        } else {
			...
			if ( 'CAPTURED' === $result->payment->card_details->status ) {
				...
				add_post_meta( $order_id, 'woosquare_transaction_id', $result->payment->id, true );
				add_post_meta( $order_id, 'square_payment_id', $result->payment->id, true );
	...
	public function process_pre_order_release_payment_woosquare( $order, $retry = true ) {
		...
		$pre_order = $order;
        $token = get_option( 'woo_square_access_token_free' );
		$woocommerce_square_settings = get_option('woocommerce_square_settings');
		$location_id = get_option('woo_square_location_id');
		$endpoint = 'squareup';
		
		if(!empty($woocommerce_square_settings)){
			if($woocommerce_square_settings['enable_sandbox'] == 'yes'){
				$location_id = $woocommerce_square_settings['sandbox_location_id'];
				$token = $woocommerce_square_settings['sandbox_access_token'];
