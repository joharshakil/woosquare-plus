...
class Square {
            ...
            if($response['account_type'] == "LOCATION"){
                ...
                if($result){
                    ...
                    foreach ($result as $location) {
                        $locations[$location['id']] = $location['location_details']['nickname'];
                    }
	...
	public function wp_remote_woosquare($url,$args,$method,$headers){
		...
		if ( !is_wp_error( $response ) ) {
			...
			update_option('wp_remote_request_woosquare_get_error_message_'.date("Y-m-d H:i:s"),$response->get_error_message());
		}
	}
