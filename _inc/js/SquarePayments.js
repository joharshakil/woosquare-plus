			...
			if ( $( '#payment_method_square' ).length ) {
			 ...
			 build_square_form(square_params);
				...
				$( document.body ).on( 'checkout_error', function() {
					...
					$( 'input.buyerVerification_token' ).remove();
				});
					...
					callbacks: {
						cardNonceResponseReceived: function( errors, nonce, cardData ) {
							if ( errors ) {
								...
								$form.append( '<input type="hidden" id="buyerVerification-token" name="buyerVerification_token" class="buyerVerification_token"  />' );
