...
<div class="squareConnectScreenFree"> 
    ...
    <div class="welcome-panel">
		...
		$query_args = array(
			...
			'scopes'   => 'MERCHANT_PROFILE_READ,ITEMS_READ,ITEMS_WRITE,PAYMENTS_READ,PAYMENTS_WRITE,CUSTOMERS_READ,CUSTOMERS_WRITE',
		);
			...
			<tbody>
				<tr>
					...
					<td>
							...
							<a href="<?php echo esc_attr( $production_connect_url ); ?>" class="wc-square-connect-button btn waves-effect waves-light btn-rounded btn-primary btnCon">
							...
							<?php } else { ?>
								<a href="<?php echo esc_attr( $disconnect_url ); ?>" class='btn waves-effect waves-light btn-rounded btn-danger btnPlu'>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 44 44" width="30" height="30">
								  <path fill="#FFFFFF" d="M36.65 0h-29.296c-4.061 0-7.354 3.292-7.354 7.354v29.296c0 4.062 3.293 7.354 7.354 7.354h29.296c4.062 0 7.354-3.292 7.354-7.354v-29.296c.001-4.062-3.291-7.354-7.354-7.354zm-.646 33.685c0 1.282-1.039 2.32-2.32 2.32h-23.359c-1.282 0-2.321-1.038-2.321-2.32v-23.36c0-1.282 1.039-2.321 2.321-2.321h23.359c1.281 0 2.32 1.039 2.32 2.321v23.36z" />
								  <path fill="#FFFFFF" d="M17.333 28.003c-.736 0-1.332-.6-1.332-1.339v-9.324c0-.739.596-1.339 1.332-1.339h9.338c.738 0 1.332.6 1.332 1.339v9.324c0 .739-.594 1.339-1.332 1.339h-9.338z" />
								</svg>    
        ...
        <div class="welcome-panel">
            ...
            <?php if ( $currencyMismatchFlag ){ ?>
                ...
                <div id="woo_square_error" class="error" style="background: #ddd;">
                    <p style="color: red;font-weight: bold;">The currency code of your Square account [ <?php echo $squareCurrencyCode ?> ] does not match WooCommerce [ <?php echo $wooCurrencyCode ?> ]</p>
                </div>
            <?php }?>
            <form method="post" <?php if ($currencyMismatchFlag): ?> style="opacity:0.5;pointer-events:none;" <?php endif; ?> >
                ...
                <p class="submit">
                    <input type="submit" value="Save Changes" class="btn waves-effect waves-light btn-rounded btn-primary">
                </p>
            </form>
         ...
         <?php if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) { ?>
            <div class="welcome-panel" <?php if ($currencyMismatchFlag): ?> style="opacity:0.5;pointer-events:none;" <?php endif; ?>>
                <a class="load-customize hide-if-no-customize btn waves-effect waves-light btn-rounded btn-secondary cusbtn" href="javascript:void(0)" id="manual_sync_wootosqu_btn" > Synchronize Woo To Square </a>
                <a class="load-customize hide-if-no-customize btn waves-effect waves-light btn-rounded btn-secondary cusbtn" href="javascript:void(0)" id="manual_sync_squtowoo_btn" > Synchronize Square To Woo </a>
                <br><br>
            </div>
        <?php  } ?>
