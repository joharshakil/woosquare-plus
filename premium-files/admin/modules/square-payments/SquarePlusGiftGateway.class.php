...
class WooSquareGiftPay_Gateway extends WC_Payment_Gateway {
	...
	public function __construct() {
		$this->id		            = 'square_gift_card_pay';
		$this->method_title 	    = __( 'Square Gift Card Pay', 'wpexpert-square' );
		$this->method_description   = __( 'Square Gift Card works by adding payments button in an woocommerce checkout and then sending the details to Square for verification and processing.', 'wpexpert-square' );
		...
		add_action( 'wp_enqueue_scripts', array( $this, 'payment_scripts_giftcardpay' ) );
		add_action( 'admin_notices', array( $this, 'admin_notices_giftcardpay' ) );
	 ...
	 */
	public function admin_notices_giftcardpay() {
		if ( ! $this->enabled ) {
			return;
		}

		// Show message if enabled and FORCE SSL is disabled and WordpressHTTPS plugin is not detected
		if ( ! WOOSQU_ENABLE_STAGING && ! is_ssl() && ! class_exists( 'WordPressHTTPS' ) ) {
	...
	public function init_form_fields() {
		...
		$this->form_fields = apply_filters( 'woocommerce_squaregiftcardpay_gateway_settings', array(
			'enabled' => array(
				'title'       => __( 'Enable/Disable', 'wpexpert-square' ),
				'label'       => __( 'Enable Square Gift Card Pay', 'wpexpert-square' ),
				'type'        => 'checkbox',
				'description' => '',
				'default'     => 'no'
			),
			'title' => array(
				...
				'default'     => __( 'Gift Card Pay (Square)', 'wpexpert-square' )
			),
			'description' => array(
	...
	public function payment_fields() { ?>
		...
		<!--<fieldset class="wooSquare-giftcard-pay">
		    <p class="form-row form-row-wide">
				<label for="sq-gift-card"><?php /*esc_html_e( 'Gift Card Pay', 'woosquare' );  */?>
				<div id="gift-card">
	 ...
	 * @access public
	 */
	public function payment_scripts_giftcardpay() {
		if ( ! is_checkout() ) {
			return;
		}
		...
		wp_register_script( 'woosquare-giftcard-pay', WooSquare_PLUGIN_URL_PAYMENT . '/js/SquarePaymentsGiftCardPay.js', array( 'jquery', 'square' ), WooSquare_VERSION, true );
		wp_localize_script( 'woosquare-giftcard-pay', 'squaregiftcardpay_params', array(
		...
		wp_enqueue_script( 'woosquare-giftcard-pay' );
		
		wp_enqueue_style( 'woocommerce-square-giftcardpay-styles', WooSquare_PLUGIN_URL_PAYMENT . '/css/SquareFrontendStyles_giftcard_pay.css' );
...
*/
	 ...
	 if(get_option('woo_square_customer_sync_square_order_sync') == "1"){
		...
		$api_client = new \SquareConnect\ApiClient($api_config);
		 ...
		 //create customer
			...
			if(function_exists('square_order_sync_add_on')){
				$data['order_id'] = square_order_sync_add_on($order,$location_id,$currency,$idempotency_key,$this->token,WOOSQU_STAGING_URL,$square_customer_id);
			}	
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		if (  'square_gift_card_pay' === ( version_compare( WC_VERSION, '3.0.0', '<' ) ? $order->payment_method : $order->get_payment_method() )  ) {
			try {
