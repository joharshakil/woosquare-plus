...
class WooSquare_Client {
	...
	public function get_api_url_base() {
		if (WOOSQU_ENABLE_STAGING) {
			return apply_filters( 'woocommerce_square_api_url',  'https://connect.'.WOOSQU_STAGING_URL.'.com/' );
		}
		...
		while ( true ) {
			...
			if ( is_object( $parsed_response['decoded_body'] ) && 
				...
				if ( $parsed_oauth_response['curl_error'] ) {
					...
					update_option( 'woo_square_access_token', sanitize_text_field( urldecode( $parsed_oauth_response['decoded_body']->access_token ) ) );
